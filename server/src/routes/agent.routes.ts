...
// Debug endpoint to test triaging with sample text
const debugTriageSchema = z.object({
  text: z.string().min(1),
  useStub: z.boolean().optional().default(false)
});
router.post('/debug/triage-test', authenticate, authorize(['admin']), async (req, res) => {
  try {
    const { text, useStub } = debugTriageSchema.parse(req.body);
    ...
    const classification = await provider.classify(text);
    
    const response = {
      input: text,
      provider: provider.isStubMode() ? 'stub' : 'gemini',
      classification,
      thresholds: {
        autoClose: env.CONFIDENCE_THRESHOLD,
        lowConfidence: env.LOW_CONFIDENCE_THRESHOLD,
        highPriority: env.HIGH_PRIORITY_THRESHOLD
      },
      autoCloseEnabled: env.AUTO_CLOSE_ENABLED,
      willAutoClose: env.AUTO_CLOSE_ENABLED && classification.confidence >= env.CONFIDENCE_THRESHOLD,
      decisionReason: classification.confidence >= env.CONFIDENCE_THRESHOLD 
        ? 'High confidence - would auto-close'
        : classification.confidence >= env.LOW_CONFIDENCE_THRESHOLD
        ? 'Medium confidence - assign to human'
        : 'Low confidence - escalate'
    };
    
    console.log('🔍 Debug triage test result:', response);
    
    res.json(response);
  } catch (err: any) {
    res.status(500).json({ error: { code: 'DEBUG_TRIAGE_FAILED', message: err.message } });
  }
});

// Get recent triaging results for debugging
router.get('/debug/recent-suggestions', authenticate, authorize(['admin', 'agent']), async (req, res) => {
  try {
    const limit = Number(req.query.limit) || 10;
    
    const recentSuggestions = await AgentSuggestion.find({})
      .sort({ createdAt: -1 })
      .limit(limit)
      .populate('ticketId', 'title description status')
      .lean();
    
    const results = recentSuggestions.map(suggestion => ({
      ticketId: suggestion.ticketId,
      predictedCategory: suggestion.predictedCategory,
      confidence: suggestion.confidence,
      autoClosed: suggestion.autoClosed,
      provider: suggestion.modelInfo?.provider || 'unknown',
      createdAt: suggestion.createdAt,
      ticket: suggestion.ticketId ? {
        title: (suggestion.ticketId as any).title,
        status: (suggestion.ticketId as any).status
      } : null
    }));
    
    console.log('📈 Recent suggestions debug:', results);
    
    res.json({
      total: results.length,
      autoClosedCount: results.filter(r => r.autoClosed).length,
      averageConfidence: results.reduce((sum, r) => sum + r.confidence, 0) / results.length,
      suggestions: results
    });
  } catch (err: any) {
    res.status(500).json({ error: { code: 'DEBUG_SUGGESTIONS_FAILED', message: err.message } });
  }
});
